...
11. Call shared `mail.search_messages` in text, summary, and raw modes, both mailbox-wide and with a folder reference. **Verify:** all initial and paged requests stay on the identical owner route, text/summary include message references, raw uses a provenance sidecar, and previews—not full bodies—remain the default.
12. Use a searched message reference with shared `mail.get_conversation`. **Verify:** conversation-ID resolution and every page stay beneath the same owner root, chronological output remains intact, text/summary messages carry references, and full bodies appear only with `output=raw`.
13. Use the same message reference with shared `mail.list_attachments` in all tiers. **Verify:** metadata carries attachment references while raw metadata remains under `data` beside provenance. Download one disposable small attachment using the matching `message_ref` and `attachment_ref`; verify size limits and base64 response behavior remain unchanged.
14. Retry attachment metadata/download with a cross-target message reference, attachment reference from another mailbox, and an attachment reference bound to another message in the same mailbox. **Pass:** every mismatch fails locally with zero Graph traffic.
