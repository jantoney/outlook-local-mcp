...
var calendarAliasMutationMu sync.Mutex
...
func replaceCalendarAliases(registry *auth.AccountRegistry, accountsPath string, entry *auth.AccountEntry, aliases []resource.CalendarAlias) (bool, error) {
	calendarAliasMutationMu.Lock()
	defer calendarAliasMutationMu.Unlock()

	oldScopes := auth.OAuthScopeUnion(auth.ScopesForMailPolicy(entry.MailPolicy), auth.ScopesForCalendarAliases(entry.CalendarAliases))
	newScopes := auth.OAuthScopeUnion(auth.ScopesForMailPolicy(entry.MailPolicy), auth.ScopesForCalendarAliases(aliases))
